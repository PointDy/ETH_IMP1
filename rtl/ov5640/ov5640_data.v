...
parameter DROP_RATE = 4'd6; // 抽帧比例：假设摄像头30fps，设为3则输出10fps；若60fps，可设为6
reg [3:0] frame_drop_cnt;

// 进行计数
always@(posedge ov5640_pclk or negedge sys_rst_n) begin
    if(sys_rst_n == 1'b0)
        frame_drop_cnt <= 4'd0;
    else if(pic_flag == 1'b1) begin 
        if(frame_drop_cnt == DROP_RATE - 1'b1)
            frame_drop_cnt <= 4'd0;
        else
            frame_drop_cnt <= frame_drop_cnt + 1'b1;
    end
end

// 只有当计数器为0时，才允许当前这一整帧的数据写入
wire frame_allow = (frame_drop_cnt == 4'd0);

//ov5640_data_out:输出16位图像数据
assign  ov5640_data_out = (pic_valid == 1'b1) ? data_out_reg : 16'b0;

//ov5640_wr_en:输出16位图像数据使能
assign  ov5640_wr_en = (pic_valid == 1'b1 && frame_allow == 1'b1) ? data_flag_dly1 : 1'b0;
